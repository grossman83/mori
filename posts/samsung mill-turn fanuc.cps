...
var subOutput = createOutputVariable({prefix:"H", control:CONTROL_FORCE}, spatialFormat);
...
function getCode(code, spindle) {
  ...
  case "UNLOCK_MULTI_AXIS":
    return (spindle == SPINDLE_MAIN) ? 69 : 69;
  case "CLAMP_CHUCK":
...
function invertAxes(activate, polarMode) {
  ...
  if (polarMode) {
    ...
    cOutput.setPrefix(activate ? "A" : "C");
    if (!yIsEnabled) {
      yOutput.disable();
    }
  }
...
function retractSubSpindle() {
  if (getProperty("useG28")) {
    ...
    writeBlock(gMotionModal.format(0), gFormat.format(53), subOutput.format(0), formatComment("SUB SPINDLE RETURN"));
  }
}
...
function onCycle() {
  if ((typeof isSubSpindleCycle == "function") && isSubSpindleCycle(cycleType)) {
    ...
    case "secondary-spindle-grab":
      if (currentSection.partCatcher) { // Activate part catcher for part cutoff section
        engagePartCatcher(true);
      }
...
function getSpindleTransferCodes(_transferType) {
  ...
  _transferCodes.spindleDirection = true; // clockwise
  if (isLastSection()) {
